...
option_list = list(
  ...
  make_option(c("--t_theta"), type="numeric", default=NULL, 
              help="theta threshold to consider pleiotropy [default= %default]", metavar="numeric"),
...
Matrix_input_file <- paste("D1_Matrices_output_pipeline_output_wo_clump.Rdata",sep="")
Matrix_extra_info_input_file <- paste("D1_Matrices_extra_info_output_pipeline_output_wo_clump.Rdata",sep="")
...
# needed to be changed back again !!!
# Parameters_needed to be read from the loaded data (which will be used for running GCPBayes)
#------------------------------------------------
# ngenes <- length(Genes_with_SNP_number_less_than_threshold)
# genes <- names(Genes_with_SNP_number_less_than_threshold)
# ngenes <- length(genes)
