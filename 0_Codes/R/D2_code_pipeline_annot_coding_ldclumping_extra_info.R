...
output_common <- c()
for (i in 1:nstud){output_common[i] <- paste("D2_output_pipeline", input_shortname[i], "common2all", nstud, "studies","coding_clumping", sep="_")}

# used for steps #2 & #3
output_names <- paste("output_pipeline", output_step4c, sep="_")

# used for steps #3 & #4
output_names_summary <- c()
for (i in 1:nstud){output_names_summary[i] <- paste("D2_Summary_SNP_in_genes", "output_pipeline", input_shortname[i], "common2all", nstud, "studies", "coding_clumping", sep="_")}

# For GWAS Annotation (used for step #2)
# directory in which the GWAS annotation data exist
path_annot_data = path_gwas_annot
# the file name of the GWAS annotation data
gwas_annot_file <- file_gwas_annot

# For LD Clumping (used for step #4)
# directory in which the LD data exist
LD_file_path = path_input_data
# the file names of the LD data
LD_file <- paste0(output_step3, ".txt")
...
for (i in 2:nstud){
  ...
  gwas_traits_others <- vroom(file = paste0(path_input_data, input[i]))
  ...
  # Finding indices of old headers in the treat_1
  header_indices_2 <- match(old_headers_2, names(gwas_traits_others))
  
  # Renaming the columns
  names(gwas_traits_others)[header_indices_2] <- new_headers_2
  
  # filtering step
  gwas_traits_others <- gwas_traits_others %>% filter(gwas_traits_others$info > info_threshold & gwas_traits_others$MAF > MAF_threshold) %>% dplyr::select(c("snp","chr","bp_hg19","Effect_A","Baseline_A","beta","se","pval","info","EAF","MAF"))
...
for (i in 2:nstud){
  ...
  # filtering step
  gwas_traits_others <- dplyr::select(filter(gwas_traits_others, gwas_traits_others$info > info_threshold & gwas_traits_others$MAF > MAF_threshold),
...
gwas_annot <- vroom(file=paste0(path_annot_data, gwas_annot_file)) %>% as.data.frame
...
gwas_common_gene_added <- inner_join(gwas_input_step2, gwas_annot, by = c ("snp" = "SNP")) %>% filter(ANNOT != ".") %>% dplyr::select(ANNOT,snp,chr,bp_hg19)
...
for (i in 1:nstud) {
  ...
  gwas_gene_added_step3 <- left_join(gwas_step3, gwas_gene_input, by = c ("snp" = "SNP")) %>% filter(gene != "NA") %>% dplyr::select(gene, snp, chr.x, pos, Effect_A, Baseline_A, beta, se, pval)
...
gwas_step5_merge <- gwas_step4[[1]] %>% dplyr::select(c(gene, snp, pos, beta, se))
colnames(gwas_step5_merge) <- c("gene","snp","pos","beta1","se1") 

for (i in 2:nstud){
  tempo_clean_gwas <- gwas_step4[[i]] %>% dplyr::select(c(gene, pos, snp, beta, se))
...
gwas_step5_merge_sumstat <- setorder(gwas_step5_merge, gene) %>% dplyr::select(-pos) %>% as.data.frame
