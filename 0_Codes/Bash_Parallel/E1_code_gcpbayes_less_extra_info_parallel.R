...
option_list = list(
  ...
  make_option(c("--t_SNP_number"), type="numeric", default=NULL, 
              help="threshold used for maximum SNP numbers", metavar="number"),
  make_option(c("--t_theta"), type="numeric", default=NULL, 
              help="threshold used for theta value to be considered as pleiotropic candidate [default= %default]", metavar="numeric"),
  make_option(c("--pathout"), type="character", default=NULL, 
              help="directory of output files", metavar="character"),
  make_option(c("--out"), type="character", default="gcpbayes_less_out.txt", 
              help="generic output file name [default= %default]", metavar="character"),
  make_option(c("--cpu"), type="numeric", default=NULL, 
              help="total number of CPUs for running the parallel process [default= %default]", metavar="numeric")
); 
...
Matrix_input_file <- paste("D3_Matrices_output_pipeline_",opt$file,".Rdata",sep="")
Matrix_extra_info_input_file <- paste("D3_Matrices_extra_info_output_pipeline_",opt$file,".Rdata",sep="")

# PATH for writing output files
path_data_write = opt$pathout

output_names <- c(paste(opt$out,"less_threshold",opt$t_SNP_number,"errors",sep="_"),
				  paste(opt$out,"less_threshold",opt$t_SNP_number,"results",sep="_"),
				  paste(opt$out,"less_threshold",opt$t_SNP_number,"pleiotropy",sep="_"))
...
no_cpu <- opt$cpu
cl <- makeCluster(no_cpu, outfile="") #not to overload your computer
print(cl)
registerDoParallel(cl)
registerDoSNOW(cl)

print("Preparing the System to run GCPBayes as Parallel")


# for a results if the gene has potential pleiotropic effects (based on its theta value)
Plei_col_names <- c("GeneIndex", "GeneName", "#SNPs", "run_time", "Theta","Pleiotropic_Effect_based_on_CI", "Pleiotropic_Effect_based_on_Median",
...
# Start running GCPBayes as parallel

result <- foreach(i=1:ngenes, .packages=c("tictoc", "GCPBayes", "BhGLM")) %dopar% {
  ...
  if (Resume_test$Criteria$theta > theta_threshold) {
    if (sum(Resume_test$Indicator$`Significant studies and Pleiotropic effect based on CI`$`Pleiotropic 
effect`=="Yes") != 0 ) {
      Pleiotropic_effect_based_on_CI <- "Yes"
    }      else {
      Pleiotropic_effect_based_on_CI <- "No"
    }
    if (sum(Resume_test$Indicator$`Significant studies and Pleiotropic effect based on median 
thresholding`$`Pleiotropic effect`=="Yes") != 0 ) {
      Pleiotropic_effect_based_on_median <- "Yes"
    }      else {
      Pleiotropic_effect_based_on_median <- "No"
    }
    
    Gene_plei <- data.frame(c(i, genename, mg, exectime, Resume_test$Criteria$theta, Pleiotropic_effect_based_on_CI, Pleiotropic_effect_based_on_median,
                              Genes_with_SNP_number_less_than_threshold_extra_info[[i]][1]$gene,
                              Genes_with_SNP_number_less_than_threshold_extra_info[[i]][2]$snp_number,
                              Genes_with_SNP_number_less_than_threshold_extra_info[[i]][3]$chr,
                              Genes_with_SNP_number_less_than_threshold_extra_info[[i]][5]$start,
