...
# If the above command returns an empty path, please install "plink" usint the following command:
...
# input file name
placo_input_file <- "output_PLACO_BCAC_2020_ALL_OCAC.txt"

# directory path for the output
path_output_data = "~/BCAC_OCAC/"
# output file name
output_file <- "output_ld_clumping_08_BCAC_2020_ALL_OCAC.txt"
...
SNPs_communs_gene_Z_P_Placo <- vroom(file=paste0(path_input_data,placo_input_file))
...
for (i in 1:22) {
  ...
  result_ld <- ld_clump(
    ...
    bfile = "~/BCAC_OCAC/1000K_ref_data/EUR"
  )
