...
option_list = list(
  ...
  make_option(c("--name1"), type="character", default=NULL, 
              help="generic name data 1", metavar="character"),
  make_option(c("--name2"), type="character", default=NULL, 
              help="generic name data 2", metavar="character"), 
...
path_gwas_sum_stat_bcac = opt$path1
path_gwas_sum_stat_OCAC = opt$path2
...
output_common <- c(paste("D1_output_pipeline",opt$name1,"common",opt$name2,"coding_withoutclumping",sep="_"),
                   paste("D1_output_pipeline",opt$name2,"common",opt$name1,"coding_withoutclumping",sep="_"))

# used for steps #2 & #3
output_names <- paste("output_pipeline",opt$out,sep="_")

# used for steps #3 & #4
output_names_summary <- c(paste("D1_Summary_SNP_in_genes","output_pipeline",opt$name1,opt$name2,"coding_withoutclumping",sep="_"),
                          paste("D1_Summary_SNP_in_genes","output_pipeline",opt$name2,opt$name1,"coding_withoutclumping",sep="_"))
...
gwas_BCAC <- select(filter(gwas_BCAC, gwas_BCAC$info > info_threshold & gwas_BCAC$MAF > MAF_threshold),
                    c("snp","chr","bp_hg19","Effect_A","Baseline_A","beta","se","pval","info","MAF"))
...
# finding common SNPs between two traits
SNP_common <- intersect(gwas_BCAC$snp, gwas_OCAC$snp)

# extracting common SNPs for the first trait
gwas_BCAC <- select(filter(gwas_BCAC, is.element(gwas_BCAC$snp, SNP_common)),
                    c("snp","chr","bp_hg19","Effect_A","Baseline_A","beta","se","pval","info","MAF"))
...
gwas_OCAC <- select(filter(gwas_OCAC, is.element(gwas_OCAC$snp, SNP_common)),
                    c("snp","chr","bp_hg19","Effect_A","Baseline_A","beta","se","pval","info","MAF"))
...
vroom_write(gwas_BCAC, file = paste0(path_output_data, output_common[1], ".txt"))
vroom_write(gwas_OCAC, file = paste0(path_output_data, output_common[2], ".txt"))
...
gwas_annot <- vroom(file=paste0(path_annot_data, gwas_annot_file,".txt"))
...
gwas_input_file_2 <- paste0(output_common[1], ".txt")
...
# finding common rows between an input file and the annotation file and adding the gene name correspond to the SNP
gwas_common_gene_added <- inner_join(gwas_input, gwas_annot, by = c ("snp" = "SNP"))

# removing the rows in which no gene was assigned to
gwas_common_gene_added <- filter(gwas_common_gene_added, ANNOT != ".")

# extracting specific columns from the "gwas_common_gene_added" 
gwas_common_gene_added <- select(gwas_common_gene_added, ANNOT, snp, chr, bp_hg19)
...
gwas_input_file_3 = c()
gwas_input_file_3[1] <- paste0(output_common[1], ".txt")
gwas_input_file_3[2] <- paste0(output_common[2], ".txt")

# the name of the file created in the #2 step of this code
gwas_SNP_in_gene_input_file <- paste0("D1_output_pipeline_SNP_in_genes_", output_names, ".txt")

# reading the "gwas_common_gene_added" file including gene name column (created in the #2 step of this code)
gwas_gene_input <- vroom(file=paste0(path_input_data, gwas_SNP_in_gene_input_file))
...
for (i in 1:length(gwas_input_file_3)) {
  ...
  gwas_gene_added_select_col_3 <- select(gwas_gene_added, gene, snp, chr.x, pos, Effect_A, Baseline_A, beta, se, pval)
...
gwas_5 = c()
gwas_5[1] <- paste0(path_input_data, output_names_summary[1], ".txt")
gwas_5[2] <- paste0(path_input_data, output_names_summary[2], ".txt")

# reading GWAS data for the first trait1
gwas5_1 <- vroom(file=gwas_5[1])
gwas5_1 <- as.data.frame(gwas5_1)

# rename "chr.x" column into "chr"
colnames(gwas5_1)[colnames(gwas5_1) == "chr.x"] <- "chr"
...
# rename "chr.x" column into "chr"
colnames(gwas5_2)[colnames(gwas5_2) == "chr.x"] <- "chr"
...
gwas5_extra_annot_columns_selected <- gwas5_extra_annot_columns[ , c("snp", "chr", "pos", 
