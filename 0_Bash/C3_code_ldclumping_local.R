...
option_list = list(
  ...
  make_option(c("--ref"), type="character", default=NULL, 
              help="directory path for the reference bfiles", metavar="character"),
  make_option(c("--r2"), type="numeric", default=NULL, 
              help="LD threshold used for the LD clumping based on r2", metavar="number"),
  make_option(c("--kb"), type="numeric", default=NULL, 
              help="Maximum distance in based pair used to consider clumping", metavar="number"),
  make_option(c("--p"), type="numeric", default=NULL, 
              help="threshold based on p", metavar="number")
); 
...
# directory path for the reference bfiles
ref_path_b_files <- opt$ref

### Thresholding parameters
# LD threshold used for the LD clumping based on r²
clump_threshold_r2 <- opt$r2
# Maximum distance in based pair used to consider clumping
clump_threshold_kb <- opt$kb
# Threshold based on p
clump_threshold_p <- opt$p
...
SNPs_communs_gene_Z_P_Placo <- vroom(file=paste0(path_input_data,placo_input_file,".txt"))
...
for (i in 1:22) {
  ...
  result_ld <- ld_clump(
    ...
    bfile = ref_path_b_files
  )
  
  if (i==1){
    write.table(result_ld, file = paste0(path_output_data, output_file),
